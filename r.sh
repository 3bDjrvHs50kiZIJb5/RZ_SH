...
v2raya_write_compose() {
  mkdir -p "$V2RAYA_DIR" "$V2RAYA_CONFIG_PATH"
...
sqlserver_write_compose() {
  mkdir -p "$SQLSERVER_DIR" "$SQLSERVER_DATA_PATH" "$SQLSERVER_BACKUP_PATH"
  local sa_password
...
services:
  sqlserver:
    ...
    ports:
      - "\\${SQLSERVER_PORT}:1433"
    volumes:
      - ${SQLSERVER_DATA_PATH}:/var/opt/mssql
      - ${SQLSERVER_BACKUP_PATH}:/var/opt/mssql/backup
EOF
}
...
uptimenode_install() {
  ...
  mkdir -p "$UPTIMENODE_DIR/data" "$UPTIMENODE_DIR/logs"
